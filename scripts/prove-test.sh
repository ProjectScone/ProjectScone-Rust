...
#
#   prove-test.sh <file> <needle> <replacement> <cargo test args...>
#
# Example:
...
cargo test "$@" >/dev/null 2>&1 || { print "   FAIL: the test does not pass to begin with"; exit 1 }
print "   yes"
...
cargo test "$@" >/dev/null 2>&1 || { print "   FAIL: restore left it broken"; exit 1 }
print "   yes"
